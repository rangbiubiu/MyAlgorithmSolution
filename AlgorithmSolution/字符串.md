...
## :fire:[8. 字符串转换整数 (atoi)](https://leetcode.cn/problems/string-to-integer-atoi/)
...
根据状态转移图建立一个覆盖所有情况的、从s与c映射到s'的状态转移表格：
...
- ------------------------------------------------------
    ...
    s[0]=`' '`，进入[0, 0] start状态（保持不变）

- s[1, 3] 都同上

- s[4]=`'-'`，进入[0, 1] signed状态，更新sign

- s[5]=`'4'`，进入 [1, 2] in_number状态，更新结果

- s[6]=`'2'`，进入 [2, 2] in_number状态，更新结果
...
1.  定义状态机，得到状态表
2.  遍历字符串取得输入每个字符后的状态，如果是`in_number`则更新结果ans，如果是`signed`则更新正负sign
    - 注意若ans超出了32位INT表示范围时需要对结果进行限制
3.  返回 sign * ans
...
class Automaton {
    ...
    // 获取下一个状态所在的列, 列索引从j=0到j=3, 分别表示【空格、符号、数字、其它】
...
public:
    int sign = 1;
    long long ans = 0;
    //获取输入每个字符后的状态，如果是in_number则更新结果，如果是符号则更新正负
    void get(char ch) { 
        // 更新state前，state是原状态，作为在StateMachine中查询的行，
        // getColumn(ch)根据原状态(行)和输入字符(列)，在StateMachine中查询下一个状态
        state = StateMachine[state][getColumn(ch)];
        if (state == "in_number") {  
            ans = ans * 10 + (ch - '0');
            ans = sign ? min(ans, (long long)INT_MAX) : min(ans, -(long long)INT_MIN);
            // 要加(long long)的原因是min的两个参数的数据类型必须一致
...
> 这一版是我自己一开始写的，被四个测试用例卡住之后自己逐步修复bug后的结果。
>
> ![image-20240528105735853](https://cdn.jsdelivr.net/gh/808bass666/picBed@main/202405281057976.png)
>
> 前面两个用例被卡住是因为没理解清楚题目的意思，没搞清楚单独的0就是一个数字而不属于前置0，以及在出现数字之前只能出现一个符号这两点。下面三个用例被卡住是因为处理舍入overflow这块没想清楚。
>
> 虽然也是有点面向测试用例编程的意思，但是我觉得逻辑还挺清晰，修复bug的逻辑也蛮清晰的。

**:key:  key：**

- 怎么处理整数超过[−2^31,  2^31−1 (2147483647) ]范围时舍入？

    一开始想的是用long long保存num, 如果num > INT_MAX, num = INT_MAX; 如果num < INT_MIN, num = INT_MIN; 

    但是！由于我们是将字符串转换为数字，字符串可以很长，因此改变num的数据类型是治标不治本

    比如"20000000000000000000"这个用例就会overflow。

    转换思路，我们可以在计算num时就提前避免overflow的情况

```cpp
int myAtoi(string s) {
/*
1. while 处理前导空格;
2. 遍历s, 对于s中每个字符s[i], 如果s[i] == 
    (1) '-' || '+' 
        - 如果num=0, 设置flag; 
        - 如果num!=0, 那就是s="0-1"这种情况, 此时读入停止, break
    (2) 数字，
        - 如果num=0且读取的数字s[i]=='0', 说明这是前导0, 跳过
        - 否则, 更新num (处理舍入要在这里进行)
    (3) 非数字字符, break
3. 处理舍入

但是还存在漏洞，
【漏洞一、s = "0-1", 预期=0, 却输出的是-1】
    当遍历到s[1]即'-'时, 按照题目本意是要break 
    但是如果像上面这样写，由于s[i]=='-' && num=0，不会break而是继续往下走，
    因此我们可以设置一个【有效数字】布尔值，表示是否已经存在有效数字
【漏洞二、s = "+-12", 预期=0, 却输出的是-12】
    和上一个漏洞一样，我们可以设置一个【有效正负符号】布尔值，表示是否已经存在有效正负符号
*/
    // 1. while 处理前导空格
    int i = 0;
    while (i < s.size() && s[i] == ' ') {
    ...
    bool havaValidNum = false; //是否已经存在有效数字
    bool havaValidSymbol = false; //是否已经存在有效数字
    for (; i < s.size(); ++i) {
        if (s[i] == '-' || s[i] == '+') {
            ...
            if (s[i] == '0' && num == 0) {
                ...
                /* 第二种做法：
                注意这里的条件！我一开始本来是写成 >= INT_MAX的, 但是
                比如num*10+(s[i]-'0')==INT_MAX且符号为负时, 即s="-2147483647"这个测试用例，
                本应返回-INT_MAX, 即-(2147483647),
                条件取等的话，就会进入下面的逻辑返回INT_MIN, 即-(2147483648) 
                */ 
...
## :fire:[165. 比较版本号](https://leetcode.cn/problems/compare-version-numbers/)
...
从左到右遍历v1和v2，分别计算它们当前正在遍历的修订号的数值（遇`'.'`停止），若这俩数值不等，则直接return，否则还需要继续遍历比较
