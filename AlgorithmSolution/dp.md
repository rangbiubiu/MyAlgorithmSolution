...
- 关键是初始化的地方很容易出错！以为dp[0]=0，实际dp[0]=1，因为第二阶可以从第0阶直接爬两阶到达，如果dp[0]=0，那么`dp[i] = dp[i - 2] + dp[i - 1] = 1`而不是2了。
- 另外就是状态压缩的代码要能流畅地写出来。
...
int climbStairs(int n) {
    // 1、确定dp数组以及下标含义:dp[i]表示爬i阶楼梯有多少种不同的方法
    // 3、dp数组初始化 
    vector<int> dp(n + 1, 1);
    // 2、确定递推公式: dp[i] = dp[i - 2] + dp[i - 1];
    // 爬i阶楼梯的方法数 =（如果是爬一步到第i阶）爬i-1阶楼梯的方法数 +（如果是爬两步到第i阶）爬i-2阶楼梯的方法数
...
1. 原题 + 要求输出每条路径。
2. 如果每次可以爬 1 或 2 或 3 个台阶呢？空间优化的写法要怎么做？
3. 如果每次可以最多可以爬 m 阶，有多少种爬法？
4. 如果某些台阶不能爬呢？（输入一个数组表示不能爬的台阶编号）
5. 要求不能连续跳两次一格。
