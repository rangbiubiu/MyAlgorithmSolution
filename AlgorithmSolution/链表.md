...
struct DoubleLinkedList { //双向循环链表 
    Node *head, *tail; //链表头、尾哨兵结点

    DoubleLinkedList() : head(new Node(-1, -1)), tail(new Node(-1, -1)){
        head->next = tail;
        tail->prev = head;
    }
    // 头插 
    void insertHead(Node* node) {
        // 先让node连在head和head->next中
        node->prev = head;
        node->next = head->next;
        // 再让head和head->next接上node
        head->next->prev = node;
        head->next = node;
    }
    // 删除链表中的结点node
...
public:
    ...
    int get(int key) {
        // 如果key存在于cache中，返回value，并将[key,value]放到链头，作为最近使用的
        // 否则直接返回-1
        if (map.count(key) == 0) {
           return -1;
        } 
        ...
        // 一种写法：直接复用put中先删除再头插结点的逻辑
        // put(key, map[key]->value);

        // 另一种写法：
        Node* node = new Node(key, map[key]->value);
        cache.deleteNode(map[key]);
        map.erase(key);
        cache.insertHead(node);
        map.insert({key, node});

        // 为什么不能直接写成如下？即只操作链表cache而不操作map？
        // cache.deleteNode(map[key]);
        // cache.insertHead(map[key]);
        // 因为双向链表和map是相辅相成的，在cache.deleteNode后map[key]就不知道指向谁了，因此需要map.erase
        
        return map[key]->value;
    }
    
    void put(int key, int value) {
        // 如果key已经存在，变更value，并将[key,value]放到链头，作为最近使用的
        // 否则将[key,value]插入链头，
        // 上面这两种情况的部分处理逻辑可以合并，即都直接头插结点即可，不同的是，key已经存在时还需要先删除结点
        // 若插入后map.size()超出capacity，尾删链表
        if (map.count(key)) {
            cache.deleteNode(map[key]);
            map.erase(key);
        } 
...
## [:fire: lc19 删除链表的倒数第 N 个结点](https://leetcode.cn/problems/remove-nth-node-from-end-of-list/) 
